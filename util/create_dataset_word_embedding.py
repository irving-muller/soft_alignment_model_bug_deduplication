...
util/create_dataset_word_embedding.py --mongo_db openOffice --collection initial
    ...
    --clean_type soft --rm_punc --sent_tok --rm_number  --lower_case
...
import pymongo
from nltk import TreebankWordTokenizer, word_tokenize, sent_tokenize, WhitespaceTokenizer
import nltk
...
from data.preprocessing import TextFieldPreprocessor, loadFilters, NonAlphaNumericalChar, NonAlphaNumCharTokenizer, \
...
parser.add_argument('--mongo_db',
                    help="Database that contains the bug reports")
...
parser.add_argument('--filters', nargs='+', help='Filter Names to be used in the summary',
                    default=['TransformLowerCaseFilter', 'TransformNumberToZeroFilter'])
...
for bug in col.find({}).max_time_ms(10000):
    ...
    sumText = cleanFunc(bug['short_desc'], args.rm_punc, args.sent_tok, args.rm_number, args.stop_words, args.stem,
                        args.lower_case, args.rm_char) if cleanFunc else bug['short_desc']
    ...
    if len(description) > 0:
        descText = cleanFunc(bug['description'], args.rm_punc, args.sent_tok, args.rm_number,
                                                args.stop_words, args.stem, args.lower_case, args.rm_char) if cleanFunc else bug['description']

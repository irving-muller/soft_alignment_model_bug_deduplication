...
from model.basic_module import SortedRNNEncoder, CNN, meanVector, lastVector, maxVector, \
...
class CADD(nn.Module):
    ...
    def __init__(self, embedding_obj, categorical_encoder, opt, summary_opt, desc_opt, matching_opt, aggregate_opt):
        ...
        if summary_opt['model_type'] in ('lstm', 'gru'):
            self.sum_extractor = SortedRNNEncoder(summary_opt["model_type"], embedding_obj, summary_opt["hidden_size"],
                                                  summary_opt["num_layers"], summary_opt["bidirectional"],
                                                  summary_opt["update_embedding"], dropout=summary_opt["dropout"])
        ...
        if desc_opt['model_type'] in ('lstm', 'gru'):
            self.desc_extractor = SortedRNNEncoder(desc_opt["model_type"], descEmbedding, desc_opt["hidden_size"],
                                                   desc_opt["num_layers"], desc_opt["bidirectional"],
                                                   desc_opt["update_embedding"], dropout=desc_opt["dropout"])
        ...
        # Matching Stage
        self.categorical_hidden = nn.Linear(categorical_encoder.getOutputSize() * 2,
                                            matching_opt['categorical_hidden_layer'])
        self.categorical_dropout = Dropout(matching_opt['categorical_dropout']) if matching_opt[
                                                                                       'categorical_dropout'] > 0.0 else None
        ...
        if matching_opt['attention'] is not None and matching_opt['attention'] != 'none':
            self.sum_sum_attention = self.create_attention_layer(matching_opt, sum_out_size,
                                                                 sum_out_size, anchor_cand_att)
            self.sum_sum_matching, sum_sum_size = self.create_matching_layer(matching_opt,
                                                                             sum_out_size)

            self.sum_sum_norm = LayerNorm(sum_sum_size) if matching_opt['layer_norm'] else None

            if matching_opt['cross_attention']:
                self.sum_desc_attention = self.create_attention_layer(matching_opt,
                                                                      sum_out_size,
                                                                      desc_out_size, True)
                self.sum_desc_matching, sum_desc_size = self.create_matching_layer(matching_opt,
                                                                                   sum_out_size)
            ...
            self.desc_desc_attention = self.create_attention_layer(matching_opt, desc_out_size,
                                                                   desc_out_size, anchor_cand_att)
            self.desc_desc_matching, desc_desc_size = self.create_matching_layer(matching_opt,
                                                                                 desc_out_size)
        ...
        if aggregate_opt['model'] == 'cnn':
            ...
            self.sum_agg = SortedRNNEncoder(aggregate_opt["model"], sum_matching_out,
                                            aggregate_opt["hidden_size"],
                                            aggregate_opt["num_layers"], aggregate_opt["bidirectional"],
                                            dropout=aggregate_opt["dropout"])
            self.desc_agg = SortedRNNEncoder(aggregate_opt["model"], desc_matching_out,
                                             aggregate_opt["hidden_size"],
                                             aggregate_opt["num_layers"], aggregate_opt["bidirectional"],
                                             dropout=aggregate_opt["dropout"])
    ...
    def create_attention_layer(self, matching_opt, input_size, output_size, bidirectional):
        return GeneralAttention(input_size, output_size, bidirectional) if matching_opt[
                                                                               "attention"] == 'general' else DotAttention(
    ...
    def forward(self, anchorInputs, candidateInputs):
        ...
        return self.similarity(anchor_sum, anchor_desc, anchor_categorical,
    ...
    def similarity(self, anchor_sum, anchor_desc, anchor_categorical, candidate_sum, candidate_desc,
        ...
        if self.sum_sum_matching:
            ...
            if self.sum_desc_attention:
                ...
                anchor_sum_desc, cand_desc_sum = self.matching(self.sum_desc_attention, self.sum_desc_matching,
                                                               anchor_sum_emb, cand_desc_emb, self.sum_desc_norm)
                cand_sum_desc, anchor_desc_sum = self.matching(self.sum_desc_attention, self.sum_desc_matching,
        ...
        categorical_comparison = F.relu(self.categorical_hidden(torch.cat([anchor_categorical * candidate_categorical,
                                                                           (
                                                                                   anchor_categorical - candidate_categorical) ** 2],
                                                                          1)))
        ...
        if agg_model == 'mean+max':
            if anchor_sum_matching_emb is not None:
                ...
                sum_agg_emb = torch.cat([meanVector(anchor_sum_matching_emb, anchor_sum_lengths),
                                         maxVector(anchor_sum_matching_emb, anchor_sum_lengths)], 1)
                desc_agg_emb = torch.cat([meanVector(anchor_desc_matching_emb, anchor_desc_lengths),
                                          maxVector(anchor_desc_matching_emb, anchor_desc_lengths)], 1)
                agg_outputs.append(sum_agg_emb)
                agg_outputs.append(desc_agg_emb)

            sum_agg_emb = torch.cat([meanVector(cand_sum_matching_emb, cand_sum_lengths),
                                     maxVector(cand_sum_matching_emb, cand_sum_lengths)], 1)
            desc_agg_emb = torch.cat([meanVector(cand_desc_matching_emb, cand_desc_lengths),
                                      maxVector(cand_desc_matching_emb, cand_desc_lengths)], 1)
            ...
            if anchor_sum_matching_emb is not None:
                # We use the comparison of the query and candidate
                # Aggregation anchor summary
                sum_agg_rnn_output, _ = self.sum_agg(anchor_sum_matching_emb, None, anchor_sum_lengths,
                                                     len(anchor_sum_lengths))
                desc_agg_rnn_output, _ = self.desc_agg(anchor_desc_matching_emb, None, anchor_desc_lengths,
                                                       len(anchor_desc_lengths))
            ...
            desc_agg_rnn_output, _ = self.desc_agg(cand_desc_matching_emb, None, cand_desc_lengths,
                                                   len(cand_desc_lengths))
            ...
            if self.pooling == 'self_att':
                ...
                agg_outputs.append(torch.cat([meanVector(sum_agg_rnn_output, cand_sum_lengths),
                                              maxVector(sum_agg_rnn_output, cand_sum_lengths)], 1))
                agg_outputs.append(torch.cat([meanVector(desc_agg_rnn_output, cand_desc_lengths),
                                              maxVector(desc_agg_rnn_output, cand_desc_lengths)], 1))
        else:
            if anchor_sum_matching_emb is not None:
                # We use the comparison of the query and candidate
                # Aggregation anchor summary
...
class SABD(nn.Module):
    ...
    def __init__(self, embedding_obj, categorical_encoder, extractor_opt, matching_opt, aggregate_opt, classifier_opt, freq=False):
        ...
        if extractor_opt['model'] in ('lstm', 'gru'):
            self.encoder = SortedRNNEncoder(extractor_opt["model"], self.field_word_size, extractor_opt["hidden_size"],
                                                  1, extractor_opt["bidirectional"])
        ...
        if extractor_opt['model'] in ('linear+lstm', 'linear+gru'):
            ...
            self.encoder = SortedRNNEncoder(rnn_type, self.field_word_size, extractor_opt["hidden_size"],
                                                  1, extractor_opt["bidirectional"])
            self.field_word_size = self.encoder.getOutputSize()
        ...
        if matching_opt is None or matching_opt["type"] == 'none':
            ...
            if attention_opt == 'dot_product':
                ...
                self.soft_alignment_attention = Tan2016Attention(self.field_word_size,
                                                                 matching_opt['attention_hidden_size'])
            if matching_opt['comparison_func'] == 'submult+nn':
                self.logger.info("===> Compare: SubMultiNN")
                self.cmp_matching_attention = SubMultiNN(self.field_word_size, self.field_word_size)
            elif matching_opt['comparison_func'] == 'mult':
            ...
            if not self.residual or not anchor_cand_att:
                raise Exception(
                    "matching_type=mean only accepted residual({}) and query+candidate({})".format(self.residual,
                                                                                                   anchor_cand_att))
            ...
            if attention_opt == 'dot_product':
                ...
                self.soft_alignment_attention = ParikhAttention(self.field_word_size,
                                                                matching_opt['attention_hidden_size'], nn.ReLU(), False,
                ...
                self.soft_alignment_attention = ParikhAttention(self.field_word_size,
                                                                matching_opt['attention_hidden_size'], None, False,
                                                                anchor_cand_att, scale)
            elif attention_opt == "query_key":
                self.logger.info(
                    "===> Attention: query_key. scale: {}; hidden_size: {}".format(scale, attention_hidden_size))
                self.soft_alignment_attention = ParikhAttention(self.field_word_size,
                                                                matching_opt['attention_hidden_size'], None, True,
                                                                anchor_cand_att, scale)
            elif attention_opt == "query_key_non_linear":
                self.logger.info(
                    "===> Attention: query_key_non_linear. scale: {}; hidden_size: {}".format(scale,
                                                                                              attention_hidden_size))
                self.soft_alignment_attention = ParikhAttention(self.field_word_size,
                                                                matching_opt['attention_hidden_size'], nn.ReLU(), True,
                                                                anchor_cand_att, scale)
            if matching_opt['comparison_func'] == 'submult+nn':
                self.logger.info("===> Compare: SubMultiNN")
                self.cmp_matching_attention = SubMultiNN(self.field_word_size, self.field_word_size)
        ...
        if aggregate_opt['model'] == 'cnn':
            ...
            self.logger.info("==> Aggregate: self_att(hidden_size={}, n_hops={})".format(aggregate_opt['hidden_size'],
                                                                                         aggregate_opt['n_hops']))
            self.agg = SelfAttention(self.field_word_size, aggregate_opt['hidden_size'], aggregate_opt['n_hops'])

            self.agg_out_size = self.agg.getOutputSize()
            self.min_sentences = 0
        elif aggregate_opt['model'] in ('lstm', 'gru'):
            self.agg = SortedRNNEncoder(aggregate_opt["model"], self.field_word_size, aggregate_opt['hidden_size'],
                                                  1, aggregate_opt["bidirectional"])
        ...
        if categorical_encoder is not None:
            ...
            self.categorical_dropout = Dropout(classifier_opt['categorical_dropout']) if classifier_opt[
                                                                                             'categorical_dropout'] > 0.0 else None
    ...
    def forward(self, anchorInputs, candidateInputs, non_candidate_input=None):
        ...
        sim_pair = self.similarity(anchor, anchor_len, anchor_categorical, anchor_mask, candidate, cand_len,
                                   candidate_categorical, cand_mask)
        ...
        sim_neg_pair = self.similarity(anchor, anchor_len, anchor_categorical, anchor_mask, non_candidate, non_cand_len,
                                       non_candidate_categorical, non_cand_mask)
    ...
    def matching(self, attention, matching_layer, anchor, candidate, anchor_mask, cand_mask):
        ...
        if attention.bidirectional:
            # Match the candidate and query
            anchor_context, candidate_context, anchor_att, candidate_att = attention(anchor, candidate, anchor_mask,
                                                                                     cand_mask)
    ...
    def similarity(self, anchor, anchor_len, anchor_categorical, anchor_mask, candidate, cand_len,
                   candidate_categorical, candidate_mask):
        # Comparison
        if self.matching_type is None or self.matching_type == 'none':
            ...
            cand_context, _ = self.soft_alignment_attention(anchor, candidate_fixed_representation,
                                                                       anchor_mask)
            anchor_context, _ = self.soft_alignment_attention(candidate, anchor_fixed_representation,
                                                                        candidate_mask)
            ...
            anchor_comparison, cand_comparison = self.matching(self.soft_alignment_attention, self.cmp_matching_attention,
                                                         anchor, candidate, anchor_mask, candidate_mask)
            if self.residual:
            ...
            # Compare the sequence others with the fixed representation. The final embedding of the report will be weighted average of these words.
            anchor_weighted_avg, anchor_att = self.soft_alignment_attention(anchor, candidate_fixed_representation,
                                                                       anchor_mask)
            cand_weighted_avg, candidate_att = self.soft_alignment_attention(candidate, anchor_fixed_representation,
                                                                        candidate_mask)
        ...
        if agg_model == 'mean+max':
            if anchor_comparison is not None:
                # We use the comparison of the query and candidate
                # Aggregation Anchor summary
                anchor_emb = torch.cat([meanVector(anchor_after_cmp, anchor_len),
                                        maxVector(anchor_after_cmp, anchor_len)], 1)

                agg_outputs.append(anchor_emb)

            cand_emb = torch.cat([meanVector(cand_after_cmp, cand_len),
                                  maxVector(cand_after_cmp, cand_len)], 1)
            ...
            if anchor_comparison is not None:
                # We use the comparison of the query and candidate
                # Aggregation anchor summary
                anchor_emb = self.agg(anchor_after_cmp, anchor_mask)[0].view(anchor_after_cmp.size(0),
                                                                                self.agg_out_size)

                agg_outputs.append(anchor_emb)

            cand_emb = self.agg(cand_after_cmp, candidate_mask)[0].view(cand_after_cmp.size(0),
                                                                           self.agg_out_size)
            agg_outputs.append(cand_emb)

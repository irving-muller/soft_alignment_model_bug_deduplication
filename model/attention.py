...
class Attention(nn.Module):
    ...
    def forward(self, similarity_matrix, encoder_seq, decoder_seq, encoder_mask, decoder_mask):
        ...
        decoder_att = F.softmax(similarity_matrix.view(-1, encoder_len), dim=1).view(batch_size, decoder_len,
                                                                                     encoder_len)
        ...
        if self.bidirectional:
            ...
            # Encoder attention
            encoder_att = F.softmax(transposed_similarity.view(-1, decoder_len), dim=1).view(batch_size,
                                                                                             encoder_len,
                                                                                             decoder_len)
...
class ParikhAttention(Attention):
    ...
    def __init__(self, input_size, hidden_size, activation=nn.ReLU(), distinct_func=False, bidirectional=False,
                 scale=None):
